...
const EXECUTION: event.Handler = [{
	...
	async onDamage(data, target) {
		...
		if (target.fainted) await this.runEvent(`Faint`, null, target);
	},
	...
	async onMove(data, target, _, sourceBattler) {
		if (!sourceBattler) return null;

		if (!data.move.verifyCorrectTargetSelection(sourceBattler, target)) return null;
		if (sourceBattler.fainted) return null;
		...
		if (data.skipSecondaryEffects !== true) {
			...
			if (result === null) data.failed = true;
		}
	},
...
const AFTER_EXECUTION: event.Handler = [{
	onMovePriority: 99,
	async onMove(data) {
		if (data.failed) await this.showText(`But it failed!`)
	}
}]
