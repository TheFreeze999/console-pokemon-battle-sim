...
const DexConditions = {
        ...
        handlers: [{
                ...
                async onAnyCheckConditionImmunity({ data, target }) {
                    if (data.condition !== DexConditions.brn)
                        return;
                    if (target.hasType(Types.Type.FIRE))
                        data.isImmune = true;
                },
        ...
        handlers: [{
                ...
                async onAnyCheckConditionImmunity({ data, target, source }) {
                    if (data.condition !== DexConditions.psn)
                        return;
                    if (source?.getAbility() === DexAbilities.corrosion)
                        return;
                    if (target.hasType(Types.Type.POISON) || target.hasType(Types.Type.STEEL))
                        data.isImmune = true;
                },
                onAnyApplyConditionPriority: 99,
                async onAnyApplyCondition({ target, data }) {
        ...
        handlers: [{
                ...
                async onAnyCheckConditionImmunity({ data, target }) {
                    if (data.condition !== DexConditions.prz)
                        return;
                    if (target.hasType(Types.Type.ELECTRIC))
                        data.isImmune = true;
                },
                onAnyApplyConditionPriority: 99,
                async onAnyApplyCondition({ target, data }) {
    ...
    leech_seeded: new Condition('leech_seeded', 'Leech Seeded', {
        handlers: [{
                onAnyCheckConditionImmunityPriority: 210,
                async onAnyCheckConditionImmunity({ data, target }) {
                    if (data.condition !== DexConditions.leech_seeded)
                        return;
                    if (target.hasType(Types.Type.GRASS))
                        data.isImmune = true;
                },
